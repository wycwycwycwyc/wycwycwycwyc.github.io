...
function uploadImage() {
    ...
    fetch(preview.src)
        ...
        .then(blob => {
            ...
            xhr.upload.onprogress = function (event) {
                if (event.lengthComputable) {
                    ...
                    progressInfo.textContent = getLocalizedText('上传进度: ', 'Upload progress: ') + percentComplete + '%';
                }
            };
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    Swal.fire(
                        getLocalizedText('图片上传成功', 'Image uploaded successfully'),
                        getLocalizedText('你的账号头像已更新', 'Your profile picture has been updated'),
                        'success'
                    );
                    location.href = '/settings.html';
                } else {
                    Swal.fire(
                        getLocalizedText('图片上传失败: ', 'Image upload failed: '),
                        xhr.statusText,
                        'error'
                    );
                }
        ...
        .catch(error => {
            ...
            Qmsg.error(getLocalizedText('无法转换图片', 'Unable to process image'));
        });
}